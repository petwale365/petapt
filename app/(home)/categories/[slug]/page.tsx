...
import { getProductsByCategorySlug } from "@/data/categories";
...
const CategoryDetailsPage = async (props: { params: Params }) => {
  const { slug } = await props.params;
  const category = await getProductsByCategorySlug(slug);
  const products = category?.products.slice(0, ITEMS_PER_PAGE);
  const productsLength = category?.products ? category.products.length : 0;
