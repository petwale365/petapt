// app/admin/collections/page.tsx
import { Suspense } from "react";
import { Plus } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Separator } from "@/components/ui/separator";
import { PageHeader } from "@/components/page-header";
import Link from "next/link";
import { createClient } from "@/utils/supabase/server";

import { DataTable } from "@/components/admin/collections/data-table";
import { Collection, columns } from "@/components/admin/collections/columns";

export const metadata = {
  title: "Collections",
  description: "Manage your product collections",
};

export default async function CollectionsPage() {
  const supabase = await createClient();

  const { data: collections, error } = await supabase
    .from("collections")
    .select(
      `
      *,
      products:product_collections(
        product:products(id)
      )
    `
    )
    .order("created_at", { ascending: false });

  if (error) {
    throw new Error(error.message);
  }

  // Add product count to each collection
  const formattedCollections =
    collections?.map((collection) => ({
      ...collection,
      product_count: collection.products.length,
    })) || [];

  return (
    <div className="flex-1 space-y-4 p-2 md:p-8 pt-6">
      <div className="flex items-center justify-between">
        <PageHeader
          title="Collections"
          description="Manage your product collections"
        />
        <Button asChild>
          <Link href="/admin/collections/new">
            <Plus className="mr-2 h-4 w-4" />
            Add New
          </Link>
        </Button>
      </div>
      <Separator />
      <Suspense
        fallback={
          <div className="flex items-center justify-center h-24">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-gray-900" />
          </div>
        }
      >
        <DataTable
          columns={columns}
          data={formattedCollections as unknown as Collection[]}
        />
      </Suspense>
    </div>
  );
}
